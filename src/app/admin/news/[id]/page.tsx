...
export default function EditStoryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setEditorData(catsData)

        // Hydrate form with legacy data handling
        setFormData({
          ...articleData,
          // Convert comma-separated strings back to arrays
          cgry_list: articleData.cgry_list ? articleData.cgry_list.split(',').map(Number).filter(Boolean) : [],
          group_list: articleData.group_list ? articleData.group_list.split(',').map(Number).filter(Boolean) : [],
          sgmt_list: articleData.sgmt_list ? articleData.sgmt_list.split(',').map(Number).filter(Boolean) : []
        })
      } catch (error) {
  ...
  const uploadArticleImage = async (articleCode: string | number, file: File) => {
    setUploading(true)
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (selectedFile) {
          try {
            await uploadArticleImage(id, selectedFile)
            setImageTimestamp(Date.now())
          } catch (uploadError) {
            console.error('Image upload failed:', uploadError)
            toast.warning('Story updated but image upload failed.')
          }
        }
        ...
        for (let i = 0; i < extraFiles.length; i++) {
          ...
          if (file) {
            try {
              ...
              extraFormData.append('filename', `full${id}-${i + 1}.jpg`)
        ...
        router.refresh()
        // Stay on page briefly then redirect to allow refresh
        setTimeout(() => router.push('/admin/news'), 500)
      } else {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
      ...
      <p className="text-slate-400 font-bold text-sm tracking-widest uppercase">Loading Story...</p>
    </div>
  )
    ...
    <form onSubmit={handleSubmit} className="pb-24">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h2 className="text-4xl font-black text-slate-900 tracking-tight">Edit Article</h2>
            <p className="text-slate-500 font-medium mt-1">Refining Story #{id}</p>
          </div>
        </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <Editor
                  ...
                  placeholder="Continue writing your news story..."
                />
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="space-y-6">
                <div
                  ...
                  className="relative p-1 border border-slate-100 rounded-3xl bg-slate-50 shadow-inner group overflow-hidden cursor-pointer"
                >
                  <input
                    type="file"
                    ref={fileInputRef}
                  ...
                  />
                    <div className="relative aspect-[16/9] rounded-3xl bg-slate-100 border border-slate-200 overflow-hidden group">
                      {previewUrl ? (
                        <img src={previewUrl} className="w-full h-full object-cover" alt="Preview" />
                      ) : id ? (
                        <FallbackImage
                          src={`${articleImageUrl(parseInt(id))}?t=${imageTimestamp}`}
                          alt="Primary"
                          fill
              ...
              {extraFiles.map((_, i) => (
                ...
                >
                  ...
                  {extraPreviews[i] ? (
                    ...
                    <div className="relative h-16 w-16 rounded-2xl overflow-hidden shadow-sm bg-white flex items-center justify-center group-hover:scale-110 transition-transform">
                      <FallbackImage 
                        src={`${articleImageUrl(parseInt(id))?.replace('-0.jpg', `-${i+1}.jpg`)}?t=${imageTimestamp}`}
                        alt={`Image ${i+1}`}
                        fill
                        className="object-cover opacity-30 group-hover:opacity-50"
                      />
                      <ImageIcon className="h-6 w-6 text-slate-200 relative z-10" />
                    </div>
                  )}
